...
def predictLung(args):
    ...
    if '.npy' in os.listdir(f'{args.dataPath}/{randomClass}'):
        for i in range(args.count):
            ...
            randomPatch = random.choice(patches)
            img = cv2.resize(randomPatch, (INPUT_SIZE, INPUT_SIZE))
            plt.imshow(img)
            plt.title(f'True label: {randomClass}')
            print(f'True label: {randomClass}')
...
def get_args():
    ...
    parser.add_argument('--modelPath', type=str, help='folder path to model')
...
if __name__ == "__main__":
    ...
    if args.modelPath is None:
        print("No model loaded. Specify the path to the model to be tested.")
    else:
